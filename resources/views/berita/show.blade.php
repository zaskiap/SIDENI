    ...
    <div class="berita-detail-wrap">
        <div class="berita-detail-content">
            ...
            <div class="berita-detail-meta">
                ...
                <span style="margin-left:12px;color:#888;">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:middle"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/></svg>
                    {{ $berita->kategori }}
                </span>
            </div>
        ...
        <aside class="berita-detail-sidebar">
            <div class="berita-terkait-card">
                <h3 class="berita-terkait-title">Berita <span class="text-red">Terkait</span></h3>
                @forelse($terkait as $t)
                    <a href="{{ route('berita.show',$t) }}" class="berita-terkait-item">
                        <div class="berita-terkait-thumb">
                            <img src="{{ $t->thumbnail_url }}" alt="{{ $t->judul }}" onerror="this.src='{{ asset('images/banner-berita.png') }}'">
                        </div>
                        <div>
                            <div class="berita-terkait-judul">{{ Str::limit($t->judul,50) }}</div>
                            <div class="berita-terkait-tgl">{{ $t->tanggal->translatedFormat('d M Y') }}</div>
                        </div>
                    </a>
                @empty
                    <p style="font-size:12px;color:#aaa">Tidak ada berita terkait</p>
                @endforelse
                <a href="{{ route('berita') }}" class="berita-terkait-all">Selengkapnya →</a>
            </div>
        </aside>
    </div>
</main>
